...
const totalElement = document.querySelector('.summary div strong')
...
function renderListCart() {
  ...
  if (cartListStorage) {
    ...
    const totalCart = cartList.reduce((acumulador, itemProduct) => {
      return acumulador + itemProduct.price
    }, 0)

    const totalFormatado = new Intl.NumberFormat('pt-br', { style: 'currency', currency: 'BRL' }).format(totalCart)

    totalElement.innerHTML = totalFormatado
